...
class RobertsCrossFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        binding.srcImageRO.setImageBitmap(Storage.bitmap)
        binding.executeButtonRO.setOnClickListener {

            (binding.srcImageRO.drawable as? BitmapDrawable?)?.let {
                binding.srcImageRO.setImageBitmap(CoreSystem.RobertsCross(it.bitmap))
            }
        }
    }
}