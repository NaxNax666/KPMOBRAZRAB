...
class SobelOperatorFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        binding.srcImageSO.setImageBitmap(Storage.bitmap)
        binding.executeButtonSO.setOnClickListener {
            (binding.destImageSO.drawable as? BitmapDrawable?)?.let {
                binding.destImageSO.setImageBitmap(CoreSystem.SobelOperator(it.bitmap))
            }
        }
    }
}