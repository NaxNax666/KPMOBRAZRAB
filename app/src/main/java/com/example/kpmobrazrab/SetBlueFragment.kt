...
class SetBlueFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.srcImageSB.setImageBitmap(Storage.bitmap)

        binding.executeButtonSB.setOnClickListener {

            val blue = binding.strenghtEditTextSB.text.toString().toInt()

            (binding.srcImageSB.drawable as? BitmapDrawable?)?.let {
                binding.srcImageSB.setImageBitmap(CoreSystem.SetBlue(it.bitmap, blue))
            }
        }
    }
}